...
* **Obsidian**：虽然足够开放，但对于“随时随地随手记”的轻量场景来说太重了（尤其是移动端）；且官方同步收费（每月 5 美元），第三方同步方案门槛和折腾成本较高。
...
Android App 基于 Expo 与 React Native 构建。用户安全连接自托管实例后，App 会在限制为实例同源的原生容器内运行该实例当前版本的移动 PWA，使 App 的 UI、编辑能力、离线草稿、同步队列、设置与导入导出流程始终跟随 PWA，而不再维护第二套独立工作区。原生桥接负责安全复用会话、外链跳转、Android 返回操作和导出文件分享。
